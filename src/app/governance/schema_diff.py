...
class SchemaDiff:
    ...
    Payload columns only (system metadata excluded).
    """

    def __init__(self, old_schema: List[Dict], new_schema: List[Dict]):
        self.old_schema = {
            f["name"]: f
            for f in old_schema
    ...
    def _classify_changes(
        ...
        for change in modified:
            ...
            elif old["mode"] == "REQUIRED" and new["mode"] == "NULLABLE":
                non_breaking.append({
                    "type": "REQUIRED_TO_NULLABLE",
                    "column": new["name"],
                })
